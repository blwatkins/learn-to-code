---
layout: post
author:
  - Brittni Watkins
date: 2026-05-28
modified_date: 2026-05-28
title: "Cryptocurrency Wallets"
tags:
  - blockchain
  - cryptocurrency
  - wallets
toc: false
---

## Introduction

Cryptocurrency wallets are software applications that provide a secure way to manage digital assets and interact with blockchain networks.
Cryptocurrency wallets do not store the cryptocurrency itself, but rather the private keys that allow users to access and manage their digital assets on the blockchain.

## Guides

## Resources and References

For additional information about cryptocurrency wallets, the following resources may be helpful:

[Coinbase Learn - What is a crypto wallet?](https://www.coinbase.com/learn/crypto-basics/what-is-a-crypto-wallet)

[YouTube: Brittni Builds - *Learning - Blockchain: Cryptocurrency Wallets* Playlist](https://youtube.com/playlist?list=PLTk8vQ-hBSzODePyyN95vSx0i1AaMg1br)
